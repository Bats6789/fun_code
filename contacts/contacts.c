...
int main( int argc, char **argv ){
    ...
    exitBox.startY = displayInfo.maxY / 2 + BOX_HEIGHT / 2;
...
int mouseOver( displayInfoType displayInfo, boxType box ){
    int exitCode;
    if( displayInfo.event->x >= box.startX && displayInfo.event->x < box.endX &&
            displayInfo.event->y >= box.startY && displayInfo.event->y < box.endY ){
        exitCode = 1;
    } else {
        exitCode = 0;
    }
